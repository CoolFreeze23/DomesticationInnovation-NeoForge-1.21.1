...
public class SinisterCarrotItem extends Item {
    ...
    public SinisterCarrotItem() {
        super(new Item.Properties().rarity(Rarity.UNCOMMON).food((new FoodProperties.Builder()).nutrition(1).saturationModifier(0.3F).effect(() -> new MobEffectInstance(MobEffects.WITHER, 100), 1.0F).build()));
    }

    public InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity entity, InteractionHand hand) {
        if(entity.getType() == EntityType.RABBIT && TameableUtils.isTamed(entity) && TameableUtils.isPetOf(player, entity) && net.neoforged.neoforge.event.EventHooks.canLivingConvert(entity, EntityType.RABBIT, (timer) -> {})){
        ...
        if(entity.getType() == EntityType.ZOMBIE_HORSE && net.neoforged.neoforge.event.EventHooks.canLivingConvert(entity, EntityType.SKELETON_HORSE, (timer) -> {})){
            player.swing(hand);
