...
public class LayerPetOverlays extends RenderLayer {
    ...
    private final ShadowHandModel SHADOW_HAND_MODEL;
    private final BlazingBarModel BLAZING_BAR_MODEL;
    private static final ResourceLocation BLAZE_TEXTURE = ResourceLocation.parse("textures/entity/blaze.png");
    private static final ResourceLocation AURA_TEXTURE = ResourceLocation.fromNamespaceAndPath(DomesticationMod.MODID, "textures/healing_aura.png");
    ...
    private static final Vector4f HEALTH_SIPHON_COLOR = new Vector4f(0.4F, 0, 0, 0.4F);
    private static final Vector4f SHADOW_BOLT_COLOR = new Vector4f(1F, 1F, 1F, 1F);

    public LayerPetOverlays(RenderLayerParent parent, net.minecraft.client.model.geom.EntityModelSet modelSet) {
        super(parent);
        this.parent = parent;
        this.SHADOW_HAND_MODEL = new ShadowHandModel(modelSet.bakeLayer(DIModelLayers.SHADOW_HAND));
        this.BLAZING_BAR_MODEL = new BlazingBarModel(modelSet.bakeLayer(DIModelLayers.BLAZING_BAR));
    }
    ...
    private static void vertex(VertexConsumer p_114090_, Matrix4f p_114091_, int p_114093_, float p_114094_, int p_114095_, int p_114096_, int p_114097_, float alpha) {
        p_114090_.addVertex(p_114091_, p_114094_ - 0.5F, (float) p_114095_ - 0.5F, 0.0F).setColor(255, 255, 255, (int)(alpha * 255)).setUv((float) p_114096_, (float) p_114097_).setOverlay(OverlayTexture.NO_OVERLAY).setLight(p_114093_).setNormal(0.0F, 1.0F, 0.0F);
    }
    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, Entity entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (TameableUtils.couldBeTamed(entity)) {
            ...
            if (TameableUtils.hasEnchant(living, DIEnchantmentKeys.IMMUNITY_FRAME) && TameableUtils.getImmuneTime((LivingEntity) entity) > 0) {
                ...
                this.getParentModel().renderToBuffer(matrixStackIn, ivertexbuilder, packedLightIn, LivingEntityRenderer.getOverlayCoords((LivingEntity) entity, 0), ((int)(alpha * 255) << 24) | 0xFFFFFF);
                matrixStackIn.popPose();
            }
            if (TameableUtils.hasEnchant(living, DIEnchantmentKeys.MAGNETIC)) {
            ...
            if (TameableUtils.hasEnchant(living, DIEnchantmentKeys.HEALTH_SIPHON)) {
            ...
            if (TameableUtils.isZombiePet(living)) {
                ...
                float alpha = 0.1F;
                matrixStackIn.pushPose();
                this.getParentModel().renderToBuffer(matrixStackIn, zombieBuffer, packedLightIn, LivingEntityRenderer.getOverlayCoords((LivingEntity) entity, 0), ((int)(alpha * 255) << 24) | 0x007F00);
                matrixStackIn.popPose();
            }
            int shadowHandCount = TameableUtils.getEnchantLevel(living, DIEnchantmentKeys.SHADOW_HANDS);
            if (shadowHandCount > 0) {
                ...
                for (int i = 0; i < shadowHandCount; i++) {
                    ...
                    SHADOW_HAND_MODEL.renderToBuffer(matrixStackIn, vertexconsumer, packedLightIn, LivingEntityRenderer.getOverlayCoords((LivingEntity) entity, 0), -1);
                    matrixStackIn.popPose();
                    matrixStackIn.popPose();
                    matrixStackIn.pushPose();
            ...
            if (TameableUtils.hasEnchant(living, DIEnchantmentKeys.BLAZING_PROTECTION)) {
                int bars = TameableUtils.getBlazingProtectionBars(living);
                float f1 = realAge * 7;
                float seperation = 360F / (TameableUtils.getEnchantLevel(living, DIEnchantmentKeys.BLAZING_PROTECTION) * 2F);
                VertexConsumer vertexconsumer = bufferIn.getBuffer(net.minecraft.client.renderer.RenderType.entityTranslucentCull(BLAZE_TEXTURE));
                matrixStackIn.pushPose();
                matrixStackIn.mulPose(Axis.YN.rotationDegrees(f));
            ...
            if (TameableUtils.hasEnchant(living, DIEnchantmentKeys.HEALING_AURA)) {
                ...
                if (t > 0) {
                    ...
                    vertex(vertexconsumer, matrix4f, 240, 0.0F, 0, 0, 1, 1.0F);
                    vertex(vertexconsumer, matrix4f, 240, 1.0F, 0, 1, 1, 1.0F);
                    vertex(vertexconsumer, matrix4f, 240, 1.0F, 1, 1, 0, 1.0F);
                    vertex(vertexconsumer, matrix4f, 240, 0.0F, 1, 0, 0, 1.0F);
                    matrixStackIn.popPose();
                }
            }
        }
